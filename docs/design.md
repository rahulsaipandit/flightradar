...
- **OpenSky coverage is uneven outside Europe/US, and noticeably sparse over India and much of Asia.** Coverage comes from crowdsourced volunteer ADS-B ground receivers, not a uniform global network — near major hubs (e.g. Delhi, Mumbai, Bengaluru) there's usually decent traffic, but rural areas can show little to nothing even with real air traffic overhead. This is a data-source limitation, not an app bug, and isn't something a client-side fix can address — it needs either a denser receiver network in that region or a different/supplementary data source.
- **Military-aircraft classification (`kmp/common/.../MilitaryAllocation.kt`) currently only covers the US DoD ICAO24 hex block.** Military aircraft from India or any other non-US country are not flagged with the military icon/filter today — they fall through to the airliner/private heuristics instead. Extending coverage means adding each country's published ICAO24 military allocation range to that table (a small, deliberately conservative list — see the file's own comment for the "high-confidence only" rationale).
